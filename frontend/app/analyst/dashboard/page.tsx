  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-gray-400 mt-1">Welcome back, {user?.name}. Here is your financial overview.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        {[
          { label: 'Total Agents',       value: loading ? '...' : summary?.agents?.total || 0,                                        color: 'text-white'      },
          { label: 'Total Transactions', value: loading ? '...' : summary?.transactions?.total || 0,                                  color: 'text-blue-400'   },
          { label: 'Total Volume',       value: loading ? '...' : `TZS ${(summary?.transactions?.volume || 0).toLocaleString()}`,     color: 'text-green-400'  },
          { label: 'Fraud Alerts',       value: loading ? '...' : summary?.fraud?.open || 0,                                          color: 'text-red-400'    },
        ].map((s) => (
